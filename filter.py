...
EXTRA_RELEVANCE = [
    ...
    # 디지털 전환·보안 인프라 (2026 실사용 용어 보강)
    #   실측 누락: 하나금융 'AX 워크숍', 농협손보 '양자내성암호/PQC',
    #             KB국민은행 '해외송금 블록체인' 등이 근거 없음으로 걸렸다.
    "디지털전환", "디지털 전환", "DX", "클라우드 전환", "마이크로서비스",
    "제로트러스트", "양자내성암호", "PQC", "동형암호", "생체인증",
    "해외송금", "간편송금", "임베디드 금융", "API 연동", "오픈API",
    "디지털 채널", "비대면 채널", "챗봇", "보이스봇", "RPA",
]
...
HARD_EXCLUDE_RE = re.compile(
    ...
    # 실적 발표 — 디지털·AI 사안이 아니고, 회사가 달라도 제목이 비슷해
    # 중복 판정을 오염시킨다("삼성카드 상반기 순익" ↔ "기업은행 상반기 순익").
    #   ※ 기존 목록엔 '순이익'만 있어 헤드라인 축약형 '순익'이 새고 있었다.
    r"(상반기|하반기|[1-4]분기|연간|누적|반기)\s*순익|순익\s*[\d,]+\s*억|"
    r"순익\s*[\d,.]+%|전년\s*(比|대비)\s*[\d.]+\s*%|"
    r"영업이익\s*[\d,]+\s*억|당기순이익\s*[\d,]+\s*억|"
